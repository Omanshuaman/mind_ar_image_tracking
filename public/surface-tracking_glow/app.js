...
var glowLayer;
let model; // Declare a global variable to store the 3D model
...
function onTouch(touchPos) {
  ...
  if (raycastResult.hit && raycastResult.pickedMesh === floor) {
    ...
    if (model) {
      // If a model exists, shift its position
      ...
      // If no model exists, load a new one
      ...
      // Start tracking on the first touch
      if (!started) {
        OX.start();
        started = true;
      }
